(() => {
  ...
  const getUsuarioActual = () => { try { return JSON.parse(localStorage.getItem('usuarioActual')); } catch { return null; } };
  ...
  const loadState = () => {
    let base;
    try { base = JSON.parse(localStorage.getItem(STORAGE_KEY)) || { ...initialState }; }
    catch { base = { ...initialState }; }
    try {
  ...
  let lastUserIdent = (() => { const u = getUsuarioActual(); return u ? (u.correo || u.nombre || null) : null; })();
    ...
    productBoxes.forEach(box => {
      ...
      boton.addEventListener('click', () => {
        ...
        if (existente) {
          ...
          state.cart.push({ nombre, precioBase: precio, precio, imagen, cantidad: 1 });
        }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if(btnVaciar){
      btnVaciar.addEventListener('click', () => {
        ensureCartSync();
        if(state.cart.length===0){ alert('El carrito ya está vacío.'); return; }
        state.cart = [];
  saveState(); // carrito limpio tras compra
    ...
    function initCheckoutPage(){
      if(window.__checkoutInitDone){ return; }
      ...
      function getCartKey(){
        const u=getUsuarioActual();
        if(u){ const ident=(u.correo||u.nombre||'guest').toLowerCase(); return CART_PREFIX+ident.replace(/[^a-z0-9._@-]/g,'_'); }
        return CART_PREFIX+'guest';
      }
      // Carga el carrito asociado al usuario actual (o invitado). No mezcla carritos.
      function loadCart(){ try { return JSON.parse(localStorage.getItem(getCartKey())) || []; } catch { return []; } }
      // Persiste un carrito (se usa principalmente al vaciar tras confirmar compra)
      function saveCart(cart){ try { localStorage.setItem(getCartKey(), JSON.stringify(cart)); } catch {} }
      // Toast visual minimalista (duplicado del global para mantener aislamiento)
      function toast(msg){ const el=document.createElement('div'); el.textContent=msg; Object.assign(el.style,{position:'fixed',inset:'auto 1rem 1rem auto',background:'#178fd6',color:'#e6ecf2',padding:'.6rem .8rem',borderRadius:'10px',zIndex:9999,border:'1px solid #178fd6'}); document.body.appendChild(el); setTimeout(()=>el.remove(),2400); }
      ...
      if(!carrito.length){
        ...
        setTimeout(()=>{ window.location.href='carrito.html'; }, 1800);
        return;
      }
      ...
      function setupEnvio(){
        ...
        if(usuario){
          ...
          const dirE = document.getElementById('env-dir'); if(dirE) dirE.textContent = principal? principal.direccion : '-';
          const comE = document.getElementById('env-comuna'); if(comE) comE.textContent = principal? (principal.comuna||'-') : '-';
          const regE = document.getElementById('env-region'); if(regE) regE.textContent = principal? (principal.region||'-') : '-';
          const telE = document.getElementById('env-tel'); if(telE) telE.textContent = principal? (principal.telefono||usuario.telefono||'-') : (usuario.telefono||'-');
      ...
      if(btnVolver){ btnVolver.addEventListener('click', ()=> window.location.href='carrito.html'); }
      const cambiarDirBtn = document.getElementById('cambiar-direccion');
      if(cambiarDirBtn){ cambiarDirBtn.addEventListener('click', ()=> window.location.href='perfil.html'); }
        ...
        btnConfirmar.addEventListener('click', ()=>{
          ...
          if(!usuario){
            ...
            if(!nombre||!email||!direccion||!comuna||!region||!telefono){
              msg.style.color='#f66'; msg.textContent='Completa todos los campos de envío.'; return; }
            datosEnvio = { nombre, email, direccion, comuna, region, telefono };
          } else {
            datosEnvio = { tipo:'usuario_registrado' };
          }
          ...
          const order = {
            id:'ORD-'+Date.now(),
            items: carrito.map(it=>({ nombre: it.nombre, cantidad: it.cantidad, unit: (it.precioBase ?? it.precio), unitFinal: esDuoc? Math.round((it.precioBase ?? it.precio)*0.8):(it.precioBase ?? it.precio) })),
            subtotal,
            descuento,
          ...
          setTimeout(()=>{ window.location.href='perfil.html'; }, 2200);
        });
      }
  ...
  const telefono = (document.getElementById('telefono')?.value || '').trim();
        ...
        const usuario = { 
          ...
          telefono,
          direcciones: [
            { id: 'DIR-'+Date.now(), etiqueta: 'Principal', direccion, comuna, region, telefono, principal: true }
          ],
  ...
  function initPerfilPage(){
    ...
    if(window.__perfilInitDone){ return; }
    ...
    let u=null; try{u=JSON.parse(localStorage.getItem('usuarioActual'));}catch{u=null;}
    if(!u){ window.location.href='usuario.html'; return; }
    const $=s=>document.querySelector(s);
    $('#nombre-usuario') && ($('#nombre-usuario').textContent = u.nombre || 'Entrenador');
    $('#nombre-completo') && ($('#nombre-completo').textContent = u.nombre || '-');
    const apSpan = document.getElementById('apellidos-usuario'); if(apSpan) apSpan.textContent = u.apellidos || '-';
    ...
    if(!Array.isArray(u.direcciones)){
      ...
      if(u.direccion || u.telefono){
        u.direcciones.push({ id:'DIR-'+Date.now(), etiqueta:'Principal', direccion:u.direccion||'', telefono:u.telefono||'', principal:true });
      }
    }
    // Garantizar que exista una dirección principal si hay al menos una
    if(!u.direcciones.some(d=>d.principal) && u.direcciones.length>0){ u.direcciones[0].principal = true; }
    ...
    function logHistorial(accion, payload){
      try {
        ...
        if(accion === 'agregar' || accion === 'marcar_principal' || accion==='eliminar'){
          ...
          const a = payload.antes || {}; const d = payload.despues || {};
        ...
        u.historialDirecciones.unshift({ id:'HIST-'+Date.now()+Math.random().toString(36).slice(2,7), fecha:ts, accion, resumen });
        ...
        b.addEventListener('click', ()=>{
          ...
          if(idx!==-1){
            ...
            if(principal){
              ...
              if(regionSpan) regionSpan.textContent = principal.region || '-';
              if(telSpan) telSpan.textContent = principal.telefono;
              u.direccion = principal.direccion; u.telefono = principal.telefono;
            } else {
      ...
      cont.querySelectorAll('button[data-action="editar"]').forEach(b=>{
        b.addEventListener('click', ()=>{
          ...
          if(d.principal){
            ...
            u.direccion = d.direccion; u.telefono = d.telefono;
          }
          logHistorial('editar', {antes:original, despues:{...d}});
          localStorage.setItem('usuarioActual', JSON.stringify(u));
          guardarUsuarios(u);
          renderDirecciones();
        });
    ...
    if(formAgregarDir){
      formAgregarDir.addEventListener('submit', (ev)=>{
        ...
        const nueva = { id:'DIR-'+Date.now(), etiqueta:et, direccion:di, comuna:co, region:re, telefono:te, principal:false };
        if(markPrincipal){
          u.direcciones.forEach(d=> d.principal = false);
          nueva.principal = true;
          u.direccion = di; u.telefono = te;
        }
        u.direcciones.push(nueva);
        if(u.direcciones.length===1){ nueva.principal = true; }
        if(nueva.principal){
        ...
        if(nueva.principal){ logHistorial('marcar_principal', nueva); }
        localStorage.setItem('usuarioActual', JSON.stringify(u));
        guardarUsuarios(u);
        renderDirecciones();
        const msg = document.getElementById('mensaje-direccion');
        if(msg){ msg.textContent = 'Dirección agregada.'; setTimeout(()=> msg.textContent='', 2200); }
    ...
    if(formPerfil){
      ...
      formPerfil.addEventListener('submit', ev=>{
        ...
        if(principalDir){ principalDir.telefono = nuevoTelP; }
        ...
        if(msgPerfil){ msgPerfil.textContent='Perfil actualizado.'; setTimeout(()=> msgPerfil.textContent='', 2200); }
      });
    }
    ...
    if(formEnvio){
      ...
      formEnvio.addEventListener('submit', (ev)=>{
        ...
        if(mensaje){ mensaje.textContent = 'Datos de envío actualizados.'; setTimeout(()=> mensaje.textContent='', 2500); }
      });
    }
  }